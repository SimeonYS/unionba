...
class UnionbaPipeline:

    # Database setup
    conn = sqlite3.connect('unionba.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `unionba`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM unionba WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO unionba (date, title, link, content)"
